...
*  Module : Compile
*  Description : _TOOL_ _TAB_ _sr_ _vl_ _ae_ _DEFAULT_
*  Involve People : -
...
(function($) {
   ...
   $.widget("ui.weekCalendar", {
      ...
      _renderCalendar : function() {
         ...
         var cboDay='<select id="cboDay" style="width:50px;">';
         for(var i=1; i<=31; i++){
           cboDay+='<option value="'+i+'">'+i+'</option>';
         }
         cboDay+='</select>';

         var cboMonthText = ['มกราคม', 'กุมภาพันธ์', 'มีนาคม', 'เมษายน', 'พฤษภาคม', 'มิถุนายน', 'กรกฎาคม', 'สิงหาคม', 'กันยายน', 'ตุลาคม', 'พฤศจิกายน', 'ธันวาคม'];
         var cboMonth='<select id="cboMonth" style="width:100px;">';
         for(var i=0; i<12; i++){
           cboMonth+='<option value="'+i+'">'+cboMonthText[i]+'</option>';
         }
         cboMonth+='</select>';

         var cboYear='<select id="cboYear" style="width:65px;">';
         for(var i=2010; i<2020; i++){
           cboYear+='<option value="'+i+'">'+i+'</option>';
         }
         cboYear+='</select>';
         ...
         if (options.buttons) {
            calendarNavHtml = "<div class=\"wc-nav\">\
                    ...
                    "+cboMonth+"\
                    "+cboYear+"\
                    <button class=\"wc-today ui-state-default ui-corner-all\">" + options.buttonText.today + "</button>\
                    <button class=\"wc-prev ui-state-default ui-corner-all\">" + options.buttonText.lastWeek + "</button>\
                    <button class=\"wc-next ui-state-default ui-corner-all\">" + options.buttonText.nextWeek + "</button>\
                    </div>";
            ...
            var cnow = new Date();
            $('#cboDay').val(cnow.getDate());
            $('#cboMonth').val(cnow.getMonth());
            $('#cboYear').val(cnow.getFullYear());

            $('#cboDay, #cboMonth, #cboYear').change(function(){
              var d = $('#cboDay').val();
              var m = $('#cboMonth').val();
              var y = $('#cboYear').val();
      ...
      _addDroppableToWeekDay : function($weekDay) {
         ...
         $weekDay.droppable({
            accept: ".wc-cal-event",
            drop: function(event, ui) {alert($weekDay);
               var $calEvent = ui.draggable;
...
//               CallSave(newCalEvent);
   ...
   $.extend($.ui.weekCalendar, {
      ...
      defaults: {
         ...
         shortMonths : ['ม.ค.', 'ก.พ.', 'มี.ค.', 'เม.ษ.', 'พ.ค.', 'มิ.ย.', 'ก.ค.', 'ส.ค.', 'ก.ย.', 'ต.ค.', 'พ.ย.', 'ธ.ค.'],
         longMonths : ['มกราคม', 'กุมภาพันธ์', 'มีนาคม', 'เมษายน', 'พฤษภาคม', 'มิถุนายน', 'กรกฎาคม', 'สิงหาคม', 'กันยายน', 'ตุลาคม', 'พฤศจิกายน', 'ธันวาคม'],
         shortDays : ['อา.', 'จ.', 'อ.', 'พ.', 'พฤ.', 'ศ.', 'ส.'],
         longDays : ['อาทิตย์', 'จันทร์', 'อังคาร', 'พุธ', 'พฤหัสบดี', 'ศุกร์', 'เสาร์']
      }
   });
