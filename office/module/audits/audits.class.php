...
//    PrintR($sql);
    ...
    $type = $this->LoadAccrType();
    foreach((array)$tmp as $i => $value){    
      $result['row'][$i]['code']=$value['code'];
      
      $result['row'][$i]['item'][]=$value['code']; 
//      $result['row'][$i]['item'][]=$type[$value['type_code']]; 
      $result['row'][$i]['item'][]=$value['name_en']; 
      $result['row'][$i]['item'][]=$value['name_th']; 
    }  

    return $result;
  }

  private function LoadCboAccrType(){
		$sql="
			SELECT
				code, 
        name_th AS name
...
//		echo $sql;
  ...
  public function LoadCbo(){
    $result['accrtype'] = $this->LoadCboAccrType();
